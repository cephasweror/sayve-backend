...
exports.analyzeGroqError = analyzeGroqError;
...
function analyzeGroqError(err, modelName) {
    ...
    if (status === 401 || rawMsg.includes('invalid_api_key') || rawMsg.includes('Unauthorized')) {
        logger_1.logger.error(`[AI] Groq authentication failed (401). Check GROQ_API_KEY. (${rawMsg})`);
        return { status: 401, isConfigError: true, isTransientError: false, message: rawMsg };
    }
    if (status === 403) {
        logger_1.logger.error(`[AI] Groq access forbidden (403) for model "${modelName}". (${rawMsg})`);
        return { status: 403, isConfigError: true, isTransientError: false, message: rawMsg };
    }
    if (status === 404 || rawMsg.includes('model_not_found')) {
        logger_1.logger.error(`[AI] Groq model not found (404) for model "${modelName}". Check GROQ_MODEL configuration. (${rawMsg})`);
    ...
    if (status === 429 || rawMsg.includes('rate_limit_exceeded')) {
        logger_1.logger.warn(`[AI] Groq rate limited (429) for model "${modelName}". Falling back to OpenRouter.`);
        return { status: 429, isConfigError: false, isTransientError: true, message: rawMsg };
    }
    if (status && status >= 500) {
        logger_1.logger.warn(`[AI] Groq server error (${status}) for model "${modelName}". Falling back to OpenRouter.`);
        return { status, isConfigError: false, isTransientError: true, message: rawMsg };
    }
    logger_1.logger.warn(`[AI] Groq request failed for model "${modelName}": ${rawMsg}`);
    return { status, isConfigError: false, isTransientError: true, message: rawMsg };
}
class PipelineService {
    ...
    initProviders() {
        ...
        const rawOpenRouterKey = process.env.OPENROUTER_API_KEY || env_1.env.OPENROUTER_API_KEY;
        ...
        logger_1.logger.info(`[AI] Gemini (Primary) configured: ${isGeminiConfigured} ${isGeminiConfigured ? `(Length: ${rawGeminiKey.length}, Prefix: "${rawGeminiKey.substring(0, 4)}***")` : ''}`);
        logger_1.logger.info(`[AI] Groq (Secondary Fallback) configured: ${isGroqConfigured} ${isGroqConfigured ? `(Length: ${rawGroqKey.length}, Prefix: "${rawGroqKey.substring(0, 4)}***")` : ''}`);
        logger_1.logger.info(`[AI] OpenRouter (Tertiary Fallback) configured: ${isOpenRouterConfigured} ${isOpenRouterConfigured ? `(Length: ${rawOpenRouterKey.length}, Prefix: "${rawOpenRouterKey.substring(0, 4)}***")` : ''}`);
    ...
    getGeminiModelName() {
        return process.env.GEMINI_MODEL || env_1.env.GEMINI_MODEL || 'gemini-2.5-flash-lite';
    }
    getGroqCandidateModels() {
        const primary = process.env.GROQ_MODEL || env_1.env.GROQ_MODEL || 'groq/compound';
    ...
    getOpenRouterCandidateModels() {
        const primary = process.env.OPENROUTER_MODEL || env_1.env.OPENROUTER_MODEL || 'meta-llama/llama-3.3-70b-instruct:free';
     ...
     */
    async callOpenRouter(systemPrompt, userPrompt, isJson = false) {
        ...
        for (const model of candidateModels) {
            try {
                ...
                if (status === 401) {
                    logger_1.logger.error(`[AI] OpenRouter authentication failed (401). Check OPENROUTER_API_KEY. (${rawMsg})`);
                    break;
                }
                else if (status === 403) {
                    logger_1.logger.error(`[AI] OpenRouter access forbidden (403) for model "${model}". (${rawMsg})`);
                }
                else if (status === 404) {
                    logger_1.logger.error(`[AI] OpenRouter model not found (404) for model "${model}". Check OPENROUTER_MODEL. (${rawMsg})`);
                }
                else if (status === 429) {
                    logger_1.logger.warn(`[AI] OpenRouter rate limited (429) for model "${model}". (${rawMsg})`);
                }
                else {
                    logger_1.logger.warn(`[AI] OpenRouter call failed for model "${model}": ${rawMsg}`);
                }
            }
        }
        return null;
    ...
    // ─────────────────────────────────────────────────────────────
    async classifyIntent(rawMessage, pendingClarification) {
        const text = (0, parser_service_1.normalizeNigerianMarketNumbers)(rawMessage).trim();
...
A pending question context may be provided. If present, strongly favor classifying as clarification_reply unless the message clearly starts a new, unrelated transaction.`;
        ...
        if (this.geminiClient) {
            ...
            logger_1.logger.info(`[AI] Trying Gemini Primary (${geminiModel})...`);
            try {
                ...
                if (intent) {
                    logger_1.logger.info(`[AI] Gemini succeeded (${geminiModel}) -> Intent: "${intent}"`);
                    return intent;
                }
            }
            catch (err) {
                ...
                if (status === 401 || status === 403 || status === 404) {
                    ...
                    logger_1.logger.error(`[AI] Gemini configuration error (${status}): ${err.message}`);
                }
                else {
                    logger_1.logger.warn(`[AI] Gemini rate limited / failed (${err.message}). Falling back to Groq...`);
                }
            }
        }
        ...
        if (this.groqClient && !isGeminiConfigError) {
            logger_1.logger.info(`[AI] Gemini unavailable or rate limited. Trying Groq fallback (${process.env.GROQ_MODEL || env_1.env.GROQ_MODEL || 'groq/compound'})...`);
            ...
            for (const model of candidateModels) {
                try {
                    ...
                    const intent = this.normalizeIntentLabel(raw);
                    if (intent) {
                        logger_1.logger.info(`[AI] Groq fallback succeeded (${model}) -> Intent: "${intent}"`);
                        return intent;
                    }
                }
                catch (err) {
        ...
        if (!isGeminiConfigError && !isGroqConfigError) {
            logger_1.logger.info('[AI] Gemini and Groq unavailable. Trying OpenRouter fallback...');
            ...
            if (openRouterRaw) {
                ...
                if (intent) {
                    logger_1.logger.info(`[AI] OpenRouter fallback succeeded -> Intent: "${intent}"`);
                    return intent;
                }
            }
        }
        // 4. Heuristic Fallback Classifier
        logger_1.logger.info(`[AI] All AI providers failed or unconfigured. Fallback heuristic classified "${text}" as "${heuristicIntent}"`);
    ...
    normalizeIntentLabel(raw) {
        ...
        if (cleaned.includes('greeting') || cleaned.includes('hello'))
            return 'greeting';
        if (cleaned.includes('setting') || cleaned.includes('currency'))
            return 'settings';
        if (cleaned.includes('delete') || cleaned.includes('undo') || cleaned.includes('remove'))
            return 'deletion';
        if (cleaned.includes('transaction'))
            return 'new_transaction';
        if (cleaned.includes('correction'))
            return 'correction';
        if (cleaned.includes('report') || cleaned.includes('summary'))
            return 'report_request';
        if (cleaned.includes('clarification') || cleaned.includes('reply'))
            return 'clarification_reply';
        return null;
    }
    heuristicClassifyIntent(lower, pendingClarification) {
        ...
        if (lower.includes('change currency') ||
    ...
    // ─────────────────────────────────────────────────────────────
    async extractTransaction(rawMessage, knownCategories = ['Sales', 'Inventory', 'Transport', 'Utilities', 'Salaries', 'Rent', 'Supplies', 'Other']) {
        const text = (0, parser_service_1.normalizeNigerianMarketNumbers)(rawMessage).trim();
...
Output: {"type":"unclear","amount":20000,"category":"Other","description":"rice and beans","needsClarification":true,"clarificationQuestion":"Is this money you spent (buying rice and beans) or money you made (selling them)?"}`;
        ...
        if (this.geminiClient) {
            ...
            try {
                ...
                if (json) {
                    logger_1.logger.info(`[AI] Gemini (${geminiModel}) successfully extracted transaction.`);
                    return {
                ...
                if (status === 401 || status === 403 || status === 404) {
                    ...
                    logger_1.logger.warn(`[AI] Gemini extraction failed: ${err.message}. Trying Groq fallback...`);
                }
            }
        }
        // 2. Groq (SECONDARY FALLBACK PROVIDER)
            ...
            for (const model of candidateModels) {
                try {
                    ...
                    if (json && (json.type || json.amount !== undefined)) {
                        logger_1.logger.info(`[AI] Groq fallback (${model}) successfully extracted transaction.`);
                        return {
                            type: json.type || 'unclear',
                            amount: typeof json.amount === 'number' ? json.amount : null,
                            category: json.category || 'Other',
        ...
        if (!isGeminiConfigError && !isGroqConfigError) {
            ...
            if (openRouterRaw) {
                try {
                    ...
                    if (json) {
                        logger_1.logger.info('[AI] OpenRouter fallback successfully extracted transaction.');
                        return {
                            type: json.type || 'unclear',
                            amount: typeof json.amount === 'number' ? json.amount : null,
                            category: json.category || 'Other',
                    ...
                    logger_1.logger.warn(`[AI] Failed to parse JSON from OpenRouter response: ${err.message}`);
                }
            }
        }
    ...
    heuristicExtractTransaction(text, knownCategories) {
        const lower = text.toLowerCase();
        ...
        let category = 'Other';
        if (lower.includes('rice') || lower.includes('sold') || lower.includes('sales'))
            category = 'Sales';
        else if (lower.includes('transport') || lower.includes('okada') || lower.includes('keke') || lower.includes('bus'))
            category = 'Transport';
        else if (lower.includes('fuel') || lower.includes('petrol') || lower.includes('diesel'))
            category = 'Fuel';
        else if (lower.includes('rent'))
            category = 'Rent';
        else if (lower.includes('salary') || lower.includes('salaries'))
            category = 'Salaries';
    ...
    // ─────────────────────────────────────────────────────────────
    async handleCorrection(rawMessage, lastTransaction) {
        const text = (0, parser_service_1.normalizeNigerianMarketNumbers)(rawMessage).trim();
...
If the correction doesn't specify a field, don't include it — the existing value stays.`;
        ...
        if (!isGeminiConfigError && !isGroqConfigError) {
            ...
            if (openRouterRaw) {
                try {
                    ...
                    logger_1.logger.warn(`Stage 2b OpenRouter diff JSON parse failed: ${err.message}`);
                }
            }
        }
        // 4. Heuristic Fallback
        const lower = text.toLowerCase();
        const diff = { provider: 'heuristic' };
        const categoryMap = [
        ...
        if (catMatch)
            diff.category = catMatch[1];
        if (lower.includes('expense') || lower.includes('spent') || lower.includes('going out'))
            diff.type = 'expense';
        else if (lower.includes('income') || lower.includes('sold') || lower.includes('coming in'))
            diff.type = 'income';
        const numbers = text.match(/\d+[\d,]*/g);
        if (numbers?.length) {
