...
export class SummaryService {
  ...
  async getSummary(user: IUser, period: 'today' | 'week' | 'month' = 'month'): Promise<string> {
    try {
      ...
      reply += `💰 *Total Income:* ${formatCurrency(totalIncome, currency)}\n`;
      reply += `💸 *Total Expenses:* ${formatCurrency(totalExpenses, currency)}\n`;
      reply += `📈 *Net Profit:* ${formatCurrency(netProfit, currency)}\n\n`;
