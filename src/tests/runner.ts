...
async function runTests() {
  ...
  const expensesMonthResult = await parserService.parseUserMessage('track my money for today');
  assert(expensesMonthResult?.isSummaryQuery === true, 'Summary: "track my money for today" detected');

  // Test 7: Clarification Question
  const clarificationResult = await parserService.parseUserMessage('600000 fuel');
